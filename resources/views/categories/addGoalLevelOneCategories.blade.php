...
@section('page_heading', Config::get('constant.ADD') . ' ' . Config::get('constant.GOAL_1'))
@section('section')
@include('alerts.errors')
@include('alerts.success')
<form action="{{action('CategoriesController@saveGoalLevelOne')}}" method="POST" class="form-horizontal margin-left-20" role="form">
    <input type="hidden" name="_token" value="<?= csrf_token();?>"/>
    <div class="form-group">
        <label class="col-sm-2 control-label padding-right-15 font-label-form" for="form-field-1"> Mã Mục Tiêu: </label>
        <div class="col-sm-10">
            <input type="text" placeholder=" vd: KH" class="col-xs-10 col-sm-5" id="txtGoalCode" name="goal_code" required>
        </div>
    </div>
    <div class="form-group">
        <label class="col-sm-2 control-label padding-right-15 font-label-form" for="form-field-1"> Tên Mục Tiêu: </label>
        <div class="col-sm-10">
            <input type="text" placeholder=" vd: Khách hàng" class="col-xs-10 col-sm-5" name="goal_name" required>
        </div>
    </div>
    <div class="form-group">
