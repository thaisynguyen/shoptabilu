...
@section('page_heading', Config::get('constant.IMPORTANT_LEVEL') . ' ' . Config::get('constant.AREA'))
@section('section')
...
$hiddenExportExcel =  ($accessLevel <= 3 && $view == 0) ? '' : 'hidden';
...
?>
    <div id="wrapper" >
        <div class="row margin-form">
            <div class="col-sm-12">
                <div class="col-sm-2 control-label padding-top-frm font-label-form text-right">Phòng/Đài/MBF HCM (*):</div>
                <div class="col-sm-3 form-group">
                    <select class="form-control combobox-99" id="cboCompany"
                            onchange="reloadPageWithParam('{{action('MajorController@managePriorityArea')}}'
                                    , 'managePriorityArea'
                                    , 'cboCompany/cboArea/cboGoal/cboYear/cboMonth')">
                ...
                <div class="col-sm-2 control-label padding-top-frm font-label-form text-right">Mục Tiêu:</div>
                <div class="col-sm-5 form-group">
                    <select class="form-control combobox-99" id="cboGoal"
                        ...
                        <option value="<?php echo $gOne->id; ?>"  <?php if($gOne->id == $selectedGoal){  echo 'selected'; } ?>>
                            <?php echo $gOne->goal_name; ?>
                        </option>
                        <?php
            ...
            <div class="col-sm-12">
                ...
                <div class="col-sm-2 control-label padding-top-frm font-label-form text-right">Tổ/Quận/Huyện:</div>
                <div class="col-sm-3 form-group">
                ...
                <div class="col-sm-1 control-label padding-top-frm padding-right-15 font-label-form text-right">Tháng (*):</div>
                <div class="col-sm-2 form-group">
                    <select class="form-control text-left combobox-99" id="cboMonth"
                            onchange="reloadPageWithParam('{{action('MajorController@managePriorityArea')}}'
                                    ...
                                    , 'cboCompany/cboArea/cboGoal/cboYear/cboMonth')">
                        <?php for($i = 1 ; $i <= 12 ; $i++) { ?>
                        <option value="<?php echo $i; ?>"  <?php if($i == $selectedMonth){  echo 'selected'; } ?>><?php echo $i; ?></option>
                        <?php } ?>
                    </select>
                </div>
            </div>
            ...
            <a role="button" id="expGoal"  class="btn btn-primary pull-right margin-btnexport {{ $hiddenExportExcel }}"
               onclick="reloadPageWithParam('{{action('ExportExcelController@exportTargetArea')}}'
                       , 'exportTargetArea'
                       , 'cboCompany/cboArea/cboGoal/cboYear/cboMonth')"><i class="fa fa-sign-out"></i> Xuất Excel</a>

            <a role="button" id="addGoal"  class="btn btn-primary pull-right change-color style-btn-addnew hidden" href="#" >
                <i class="fa fa-plus"></i> Thêm Mới</a>
            <table id="tblArea" class="table-common" style="margin-top: 100px;">
                <thead>
                ...
                ?>
                            <tr style="background-color: #D8E4BC">
                                ...
                                <td class="table-icon text-center">
                                    <a href="{{url('updatePriorityArea').'/'.$row->id}}" role="button" title="Cập nhật">
                                ...
                                <td class="table-icon text-center" >
                                    ...
                                    @include('popup.confirmDelete', array('rowId' => $row->id
                                                                  , 'data' => $row->id
                                                                  , 'strName' => $row->goal_name.'<br/><br/><span style="color:red;">* Chú ý:</span> Sau khi xóa thì Tỷ trọng/Kế hoạch/Thực hiện của Tổ/Quận/Huyện, Chức danh, Nhân viên đối với mục tiêu: '.$row->goal_name.' áp dụng tháng: '.$row->month.'/'.$row->year.' sẽ bị xóa'
                                                                  , 'actionName'  => 'MajorController@deletePriorityArea'
                                                                  ))
                                </td>
                                <?php
                                }
                            ...
                            <tr style="background-color: white">
                                ...
                                ?>
                                <td class="table-icon text-center">
                                    <a href="{{url('updatePriorityArea').'/'.$child->id}}" role="button" title="Cập nhật">
                                        <i class="fa fa-pencil-square-o"></i>
                                    </a>
                                </td>
                                ...
                                <td class="table-icon text-center" >
                                    ...
                                    @include('popup.confirmDelete', array('rowId' => $child->id
                                                                  , 'data' => $child->id
                                                                  , 'strName' => $child->goal_name.'<br/><br/><span style="color:red;">* Chú ý:</span> Sau khi xóa thì Tỷ trọng/Kế hoạch/Thực hiện của Tổ/Quận/Huyện, Chức danh, Nhân viên đối với mục tiêu: '.$child->goal_name.' áp dụng tháng: '.$child->month.'/'.$child->year.' sẽ bị xóa'
                                                                  , 'actionName'  => 'MajorController@deletePriorityArea'
                                                                  ))
                                </td>
                            </tr>
                            ...
                            ?>
                                    <tr style="background-color: white">
                                        ...
                                        <?php
                                        $str = $selectedCompany.','.$selectedArea.','.$selectedGoal.','.$selectedYear.','.$selectedMonth.','.$child->id.','.$child->goal_id.',1';
                                        ?>
                                        <td class="table-icon text-center" >
                                            <a role="button" title="Xóa" style="cursor: pointer;" data-target=".dialog-delete-goal-<?php echo $child->id; ?>" data-toggle="modal">
                                                <i class="fa fa-trash"></i>
